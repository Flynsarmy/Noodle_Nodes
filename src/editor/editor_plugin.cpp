...
void NNEditorPlugin::_bind_methods() {
	ClassDB::bind_method(D_METHOD("on_selection_changed"), &NNEditorPlugin::_on_selection_changed);
}

// Godot virtuals.

/**
void NNEditorPlugin::_enter_tree() {

}


void NNEditorPlugin::_exit_tree() {

}
/**/

void NNEditorPlugin::_notification(int p_what) {
	switch (p_what) {
		case NOTIFICATION_ENTER_TREE: {
			...
			Ref<PackedScene> scene_template = godot::ResourceLoader::get_singleton()->load("res://addons/noodle_nodes/editor/editor.tscn");
			...
			//ERR_FAIL_V_MSG( _editor_scene == nullptr, "NNEditorPlugin::_enter_tree() - Failed to create the editor interface.");
			...
			// Add the debugger.
			_debugger = Ref<NNDebuggerPlugin>(memnew(NNDebuggerPlugin));
...
void NNEditorPlugin::_on_selection_changed() {
...
void NNEditorPlugin::btree_add_child_nodes(Tree *tree, TreeItem *parent_node, NNBTNodes *parent_ai_node) {
